...
def run():
    ...
    if prompt:
        ...
        #adding URL's content to conversation hhistory
        ...
        #intialize assistant response
        assistant_response = ""

        if(Conversation_memory =="buffer of 5 questions"):
            conversation_history.extend(st.session_state.messages[-5:])
        if llm_option == "OpenAI" :
